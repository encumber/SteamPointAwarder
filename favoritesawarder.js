  ...
  if (pointType === "cheap") {
    ...
    */
    ...
    function getAwardCounts(steamID, type) {
        ...
        return new Promise((resolve, reject) => {
            $http.getText(`https://steamcommunity.com/profiles/${steamID}/${subPath}?appid=0&browsefilter=myfavorites`)
                .then((text) => {
                    try {
    ...
    function getAwardItems(steamID, type, p = 1) {
        ...
        switch (type) {
            case 's':
                subPath = `screenshots/?p=${p}?appid=0&browsefilter=myfavorites&view=grid&l=english`;
                preg = /id="imgWallHover(\d+)"/g;
                break;
            case 'i':
                subPath = `images/?p=${p}?appid=0&browsefilter=myfavorites&view=grid&l=english`;
                preg = /id="imgWallHover(\d+)"/g;
                break;
            default:
                throw 'type error';
